...
public final class GuardService extends AccessibilityService {
 ...
 @Override protected void onServiceConnected(){super.onServiceConnected();ref=new WeakReference<>(this);if(controller!=null)controller.close();controller=new GuardController(this);if(new Prefs(this).enabled())ProtectionNotificationService.start(this);ProtectionNotificationService.statusChanged();}
 ...
 private void cleanup(){if(controller!=null){controller.close();controller=null;}ref=null;ProtectionNotificationService.statusChanged();}
